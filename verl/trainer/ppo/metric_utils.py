...
def calculate_pass_k(correct_count: int, total_count: int, k: int) -> float:
    """
    Calculate the pass@k metric.
    
    The pass@k metric represents the probability of getting at least one correct answer
    in k independent generations, given c correct answers out of n total samples.
    
    Formula: pass@k = 1 - C(n-c, k) / C(n, k)
    
    Args:
        correct_count: Number of correct answers (c)
        total_count: Total number of samples (n)
        k: Number of independent generations to consider
        
    Returns:
        The pass@k metric value
    """
    if k > total_count:
        k = total_count
    ...
    if total_count - correct_count < k:
        return 1.0  # 至少有一个正确答案肯定会被采样到

    numerator = math.comb(total_count - correct_count, k)
    denominator = math.comb(total_count, k)
...
def process_validation_metrics(data_sources: list[str], sample_inputs: list[str], infos_dict: dict[str, list[Any]], seed: int = 42) -> dict[str, dict[str, dict[str, float]]]:
    ...
    Returns:
        ...
        - "pass@N": Pass@k metric for different values of N
            ...
            for var_name, var_vals in var2vals.items():
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        ...
                        # Convert scores to binary correct/incorrect based on threshold (assuming scores > 0 are correct)
                        binary_scores = [1 if score > 0 else 0 for score in var_vals]
                        correct_count = sum(binary_scores)
                        metric[f"pass@{n}"] = calculate_pass_k(correct_count, n_resps, n)
