...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences_by_entropy(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Prepare batched inputs for all high entropy positions
        batched_vllm_inputs = []
        input_metadata = []  # Store metadata to map outputs back to prompts
        ...
        for prompt_idx in range(batch_size):
            ...
            # Create prompts for all high entropy positions at once
            for pos in high_entropy_positions:
                ...
                batched_vllm_inputs.append(new_vllm_input)
        ...
        # Generate all continuations in a single batch
        if batched_vllm_inputs:
            # Prepare lora requests for the batch if needed
            batched_lora_requests = None
            if lora_requests:
                batched_lora_requests = []
                for meta in input_metadata:
                    batched_lora_requests.append(lora_requests[meta["prompt_idx"]])
            
            # Generate all continuations in a single batch
            with self.update_sampling_params(**sampling_params_step1):
                batched_outputs = self.inference_engine.generate(
                    prompts=batched_vllm_inputs,
                    sampling_params=self.sampling_params,
                    lora_request=batched_lora_requests,
                    use_tqdm=False,
                )
            
            # Process all outputs
            for output_idx, output in enumerate(batched_outputs):
                meta = input_metadata[output_idx]
                prompt_idx = meta["prompt_idx"]
                pos = meta["pos"]
                prefix_response_ids = meta["prefix_response_ids"]
                ...
                # Ensure we have lists to append to
                while len(all_responses) <= output_idx:
                    all_responses.append([])
                    all_log_probs.append([])
                
                # Append the new response and its log probs
                if len(all_responses) <= output_idx + 1:
                    all_responses.append([None] * batch_size)
                    all_log_probs.append([None] * batch_size)
                
                all_responses[output_idx + 1][prompt_idx] = prefix_response_ids + continuation_ids
                all_log_probs[output_idx + 1][prompt_idx] = first_log_probs[prompt_idx][:pos] + continuation_log_probs
