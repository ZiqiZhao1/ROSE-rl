...
class ChatCompletionScheduler:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        if batch.meta_info.get("validate", False):
            kwargs["top_p"] = batch.meta_info.get("top_p", self.config.val_kwargs.top_p)
            kwargs["temperature"] = batch.meta_info.get("temperature", self.config.val_kwargs.temperature)
            if hasattr(self.config.val_kwargs, "top_k"):
                kwargs["top_k"] = batch.meta_info.get("top_k", self.config.val_kwargs.top_k)
