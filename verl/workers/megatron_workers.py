...
class ActorRolloutRefWorker(MegatronWorker, WorkerProfilerExtension):
    ...
    @GPUMemoryLogger(role="generate_sequences_by_entropy", logger=logger)
    @WorkerProfiler.annotate(color="red")
    def generate_sequences_by_entropy(self, prompts: DataProto):
        assert self._is_rollout
        if self._is_offload_param:
            load_megatron_model_to_gpu(self.actor_module)
            log_gpu_memory_usage("After load actor params during generate_sequences_by_entropy", logger=logger)
        ...
        with self.sharding_manager:
            ...
            with simple_timer("generate_sequences_by_entropy", timing_generate):
                output = self.rollout.generate_sequences_by_entropy(prompts=prompts)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @GPUMemoryLogger(role="generate_sequences_by_iterative_entropy", logger=logger)
    @WorkerProfiler.annotate(color="red")
    def generate_sequences_by_iterative_entropy(self, prompts: DataProto):
        assert self._is_rollout
        if self._is_offload_param:
            load_megatron_model_to_gpu(self.actor_module)
            log_gpu_memory_usage("After load actor params during generate_sequences_by_iterative_entropy", logger=logger)
        prompts.batch = prompts.batch.to(get_device_name())
        meta_info = {
            "eos_token_id": self.generation_config.eos_token_id if self.generation_config is not None else self.tokenizer.eos_token_id,
            "pad_token_id": self.generation_config.pad_token_id if self.generation_config is not None else self.tokenizer.pad_token_id,
