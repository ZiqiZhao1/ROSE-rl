...
class ActorRolloutRefWorker(Worker, WorkerProfilerExtension):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            with simple_timer("generate_sequences", timing_generate):
                use_entropy_branching = self.config.rollout.get("use_entropy_branching", False)
                use_loop_sampling = self.config.rollout.get("use_loop_sampling", False)
                entropy_branching_mode = self.config.rollout.get("entropy_branching_mode", "none")
                sys.stdout.flush()
                # print(f"[DEBUG] use_entropy_branching: {use_entropy_branching}")
                # print(f"[DEBUG] entropy_branching_mode: {entropy_branching_mode}")
                if use_entropy_branching and entropy_branching_mode == "iterative":
                    output = self.rollout.generate_sequences_loop_entropy(prompts=prompts)
                elif use_entropy_branching:
                    raise NotImplementedError("Single-branch entropy sampling is not supported")
                else:
                    if use_loop_sampling:
                        output = self.rollout.generate_sequences_loop(prompts=prompts)
                    else:
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @WorkerProfiler.annotate(color="red")
    def generate_sequences_normal(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_device_id())
        ...
        with self.rollout_sharding_manager:
            ...
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            with simple_timer("generate_sequences_normal", timing_generate):
                output = self.rollout.generate_sequences(prompts=prompts)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @WorkerProfiler.annotate(color="red")
    def generate_sequences_by_iterative_entropy(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_device_id())
        ...
        with self.rollout_sharding_manager:
            ...
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            with simple_timer("generate_sequences_by_iterative_entropy", timing_generate):
                output = self.rollout.generate_sequences_by_iterative_entropy(prompts=prompts)
